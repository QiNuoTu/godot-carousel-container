...
class_name HWaveContainer
## 横向波浪形轮播布局容器，提供平滑动画和丰富的视觉效果[br][br]
## HWaveContainer是一种将控件根据波浪轨迹排列和切换的容器
...
func _update_child_position(child: Control, delta: float) -> void:
	...
	var x = distance_from_selected * wave_spacing
	var phase = float(child_index) / max(1, child_count - 1) * TAU * wave_frequency
	var y = sin(phase) * wave_amplitude
