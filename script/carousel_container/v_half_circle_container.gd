...
class_name VHalfCircleContainer
## 纵向半圆轮播布局容器，提供平滑动画和丰富的视觉效果[br][br]
## VHalfCircleContainer是一种将控件根据半圆纵向排列和切换的容器

@export_group("Wraparound")
## 环绕的弧度范围
@export_range(-PI, PI, 0.001, "or_greater", "or_less") var wraparound_radian: float = PI
## 环绕半径
...
func _update_child_position(child: Control, delta: float) -> void:
	...
	var y = sin(angle) * (wraparound_radius + spacing_adjustment)
