...
class_name HLinearContainer
## 横向轮播布局容器，提供平滑动画和丰富的视觉效果[br][br]
## HLinearContainer是一种将控件根据直线横向排列和切换的容器
...
func _update_child_position(child: Control, delta: float) -> void:
	...
	var target = Vector2((child_index - selected_index) * (child.size.x + spacing), 0) - child.size * 0.5
	if !child.position.is_equal_approx(target):
		child.position = lerp(child.position, target, smoothing_speed * delta)
