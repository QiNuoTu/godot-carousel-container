...
class_name HHalfCircleContainer
## 横向半圆轮播布局容器，提供平滑动画和丰富的视觉效果[br][br]
## HHalfCircleContainer是一种将控件根据半圆横向排列和切换的容器

@export_group("Wraparound")
## 环绕的弧度范围
@export_range(-PI, PI, 0.001, "or_greater", "or_less") var wraparound_radian: float = PI * 0.4
## 环绕半径
@export var wraparound_radius: float = 300.0
## 环绕高度
@export var wraparound_height: float = 20.0
## 子项之间的间距
...
func _update_child_position(child: Control, delta: float) -> void:
	...
	var max_index_range = max(1, (child_count - 1) * 0.5)
	var angle = clamp(distance_from_selected / max_index_range, -1.0, 1.0) * wraparound_radian
	var spacing_adjustment = distance_from_selected * spacing if wraparound_off_centre else spacing
	var x = sin(angle) * (wraparound_radius + spacing_adjustment)
	var y = cos(angle) * wraparound_height
	var target_pos = Vector2(x, y - wraparound_height) - child.size * 0.5
